...
#if !defined(LANG_R) && defined(_WIN32) && !defined(__MINGW32__)
	...
	uint32_t *iter, double *dGap, double *info_list)
{
	...
	for (; NewtonIter <= maxNewtonIter; NewtonIter++)
	{
		...
		else
		{
			...
			for (unsigned long cdSweep = 1; cdSweep <= 2000; cdSweep++)
			{
				diffD = 0.0;
		...
		// double *X_t = (double *)malloc(p * p * sizeof(double));
		// memcpy(X_t, X, sizeof(double) * p * p);
